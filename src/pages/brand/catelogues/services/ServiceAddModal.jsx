...
const ServiceAddModal = (props) => {
  ...
  const {
    ...
    serviceSaveError,
    singleService,
    serviceSaveUpdateLoading,
    serviceSaveUpdateMessage,
    serviceSaveUpdateError,
    dropDownCatalogue,
  ...
  useEffect(() => {
    if (!serviceSaveLoading && serviceSaveMessage) {
      ToastService.success(serviceSaveMessage);
  ...
  useEffect(() => {
    if (!serviceSaveUpdateLoading && serviceSaveUpdateMessage) {
      ToastService.success(serviceSaveUpdateMessage);
      props.setIsShow(!props.show);
      dispatch(resetFormUpdate());
    }
    // eslint-disable-next-line react-hooks/exhaustive-deps
  ...
  useEffect(() => {
    if (singleService !== null) {
      const newBranches =
        singleService &&
        singleService.branch_id &&
        singleService.branch_id.map((o) => {
          const newBranch =
            branches &&
            branches.find((obj) => {
              return obj._id === o;
      ...
      const newExperienceType =
        singleService &&
        singleService.experience_type_id &&
        singleService.experience_type_id.map((o) => {
          const newExp =
            experienceTypeData &&
      ...
      formik.setValues({
        ...
        for_sale: singleService.for_sale,
        category_id: singleService.service_catalogue_id,
      });
    } else {
      formik.setValues({
        language: "en",
    ...
    // eslint-disable-next-line react-hooks/exhaustive-deps
  }, [singleService, branches, experienceTypeData]);
  ...
  const handleSubmit = (values) => {
    ...
    if (singleService !== null) {
      const selectId = singleService._id;
      formData.append("service_catalogue_id", values.category_id);
      dispatch(
        updateService(formData, selectId, singleService.service_catalogue_id)
      );
    } else {
      dispatch(addService(formData, values.category_id));
    }
  };
  ...
  return (
    <Modal show={props.show} size="lg">
      {serviceSaveError && <ErrorList error={serviceSaveError} />}
      {serviceSaveUpdateError && <ErrorList error={serviceSaveUpdateError} />}
      <Modal.Header>
        <Modal.Title className="cust-title">
          {singleService ? t("UPDATE_SERVICE") : t("ADD_NEW_SERVICE")}
        </Modal.Title>
        <div>
          ...
          <div className="flex">
            <div className="modalAddBranchLeft">
              ...
              <div className="form-group">
                <Form.Label htmlFor="text">{t("SERVICE_NAME")}</Form.Label>
                <Form.Control
                  type="text"
                  ...
                  placeholder={t("INPUT_SERVICE_NAME")}
              ...
              <div className="form-group">
                ...
                <Form.Control
                  as="textarea"
                  rows={3}
                  placeholder={t("INPUT_SERVICE_DESCRIPTION")}
              ...
              <div className="form-group">
                ...
                <Form.Control
                  type="text"
                  placeholder={t("INPUT_SERVICE_PRICE")}
                  ...
                  onChange={formik.handleChange}
                  onBlur={formik.handleBlur}
                  disabled={formik.values.for_sale === true ? true : false}
                />
              ...
              <div className="catalogueModal">
                <div className="d-flex mb-4">
                  ...
                  <Form.Label className="mb-0 ms-2">
                    {t("THIS_SERVICE_IS_NOT_FOR_SALE")}
                  </Form.Label>
                </div>
                <div className="form-group ">
